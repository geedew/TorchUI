...
var torchConfig = {
	...
	/*** These settings will adjust all of the dialogs for you if using jQueryUI ***/
			...
			dialogSettings: {
					...
					create : function(elm) {
						...
						/* this function will run when the dialog is created */
						//var elmContent = jQuery(elm).find(".ui-dialog-content:first");
						/* Add the prepended content */
						jQuery(elmContent).prepend(jQuery("<p />").addClass("offscreen").html("This dialog is shown above the content on the page to visual user. If using a Screen Reader, please update your Screen Reader's buffer for a consistent experience. Press Escape to close this dialog."))
						/* Add the appended content */
						jQuery(elmContent).append(jQuery("<a />").addClass("offscreen").html("Jump back to top of the dialog.").attr("href","#top").click(function() { jQuery(elm).find("a:first").focus(); return true;}));
						return;
					},
					focus : function() { 
	...
	/*!! Preloaded Content !!*/
		...
		preloadContent : '<img src="/images/indicator.gif" alt="Please wait while the page is loading..." />',
	/*!! METADATA !!*/
...
/* Deprecating for use of jQuery.inArray */
...
/* getScript needs to have the option for caching, this little trick just overwrites it to allow the flag. */
...
/* We want to use a case insenstive search normally */
/*Case insensitive contains search */
(function($) { $.extend($.expr[':'].contains = function(a,i,m){ return (a.textContent || a.innerText || "").toLowerCase().indexOf(m[3].toLowerCase())>=0; }); })(jQuery);

/* Add a Delay function for use in the code */
/* Deprecated by using jQuery 1.4.2 version *//*
(function($) {
...
/* Metadata will allow the use of html-5 attributes, but make them accessible via the .data() element call. */
/* Metadata.js */
/*
	...
	$.extend({
		torchui : {
			...
			create : function() {
				 ...
				 * Metadata will from now on only use the HTML5 data-* equivalents! */
				...
				if($.torchui.defaults.usejQueryUI) {
					...
					$.torchui.require($.torchui.path+ "js/jquery-ui-1.8.2.custom.min.js", { type: "js"});
				}
...
/*NEEDS A TIMEOUT!!!!*/	
	...
	/* Load the defaults */
	$.extend($.torchui.defaults, torchConfig);
})(jQuery);



/** Firebug Lite **/
/* This function works by creating a fake entry for all of the firebug lite commands, and hten loading firebug lite
 * when it it needed so that $.console, etc can be used in all browsers, whethey they support it or not. */
...
//                    }
	...
	if(!$.torchui.defaults.debug) {
		/* This will make sure that any usage of firebug does not break browsers that do not support console*/
		$.firebug.off();
	} else {
		/* Check to see if firebug lite is also turned on */
...
/**
  * TorchUI Buffer manager.
  * Allows ajax calls to be queued and buffered accordingly.
  */
		...
		$.fn.extend({
			opendialog : function() {
				...
				return this.each(function() {
					...
					/* Fire off the create dialog function to manage the buttons */
					settings.create($(this).dialog("widget"));
				});
			}
		});
				...
				if((!$.torchui.forms[this] || settings.reload)) {
					...
					if($.torchui.defaults.maskForms) {
						...
						$($.torchui.defaults.maskConfig).each(function() {
							/* Not sure what this is doing, but causing error */
							/* TDOD - fix this function
