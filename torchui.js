...
var torchConfig = {
	...
	/*** This will turn the next four features on or off, if they are all set to true ***/
		...
		validateForms : true,
			 ...
			 *   Just give a field the class and autoForms to true will automatically add this validation
			 *   All error text is in the lang.js file!
			 * phone - validates all US phone numbers with area code 
			 * postalCode - validates all US postal codes 
			 * email - validates valid emails 
			 * date - validates valid dates (all javascript dates) 
			 * url - validates a valid url 
			 * min[#] - the number must be at least #
			 * max[#] - the number must be less than #
			 * range[#-#] - the number must be between
			 * minlength[#] - at least this(#) long
			 * maxlength[#] - no longer than #
			 */
	 ...
	 */
		...
		ajaxForms : true, //  ajax the forms class="notajax" turns off.
	...
	$.extend({
		torchui : {
			...
			require : function(path) {
				...
				$.extend( options, arguments[1] ); // pass in any options
				...
				if(options.type == "js") {
					try {
						...
						if($.makeArray(this.scripts).indexOf(path) == -1 || options.reload) {
							
							/* Add the script to the list of laoded only if not previously loaded */
							if(!options.reload || $.makeArray(this.scripts).indexOf(path) == -1) $.torchui.scripts.push(path);
							
							
							if($.browser.safari) {/* Throws an XML not well-formed error  */
								var script = $("<script/>")
								if(options.id) $(script).attr("id",options.id);
	...
	if($.torchui.defaults.usejQueryUI) {
		...
		$(document).ready(function() {
			$("div.dialog").each(function() {
				if($(this).hasClass("nobuttons")) defaults.buttons = null;
				var title = $(this).children("h1.title");
				if(title.length > 0) {
					defaults.title = $(title).html();
					$(title).hide();
				}
				$(this).dialog(defaults);
			});
		...
		$.opendialog = function(value) {
			...
			$.extend(defaults, arguments[1], { autoOpen : true });
			return $($(defaults.elem).html(value)).dialog(defaults);
		}		
	} else {
...
(function($) {
	...
	$.forms = function() {
		...
		try {
			/* Loop every element that is being found */
			$(options.current).each(function() {
				/* check for unloading or reloading of the form */
				if(options.unload || options.reload) {
					unload(this);
					if(options.unload) return true; }
